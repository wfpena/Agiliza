...
{
    public abstract class AgilizaScrumTestBase : AbpIntegratedTestBase<AgilizaScrumTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected AgilizaScrumTestBase()
        {
        ...
        protected void UsingDbContext(Action<AgilizaScrumDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<AgilizaScrumDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<AgilizaScrumDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<AgilizaScrumDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<AgilizaScrumDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<AgilizaScrumDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<AgilizaScrumDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<AgilizaScrumDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<AgilizaScrumDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<AgilizaScrumDbContext, Task<T>> func)
        {
            T result;
